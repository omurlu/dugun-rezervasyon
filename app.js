...
function reservationListMarkup(query = "") {
  const filtered = state.reservations.filter(item => reservationMatches(item, query));
...
function dashboardSearchMarkup(query = "") {
  ...
  if (!needle) return "";
  const filtered = state.reservations.filter(item => reservationMatches(item, needle));
  return `
...
function renderDashboard() {
  const eventMap = state.reservations.reduce((map, item) => {
      ...
      <form id="detailedReservationForm">
        <section class="panel compact-panel">
          ...
          <div class="form-grid thirds">
            ...
            <div class="field"><label>Organizasyon Türü *</label><select name="type" required>${selectOptions(state.organizationTypes, editing?.type || "", "Seçiniz")}</select></div>
        ...
        <section class="panel compact-panel">
          ...
          <div class="form-grid">
            <div class="field"><label>Salon *</label><select name="hallId" required>${selectOptions(state.halls, selectedHall, "Salon")}</select></div>
            <div class="field"><label>Paket</label><select name="packageId" id="quotePackage">${selectOptions(state.packages, selectedPackage, "Paket seçin")}</select></div>
        ...
        <section class="panel compact-panel">
          ...
          <div class="choice-list">
            ${state.extras.map(item => checkboxPriceRow("extras", item.id, item.name, item.price)).join("")}
          </div>
        </section>
        ...
        <section class="panel compact-panel">
          ...
          ${state.menuCategories.map(category => {
            const items = state.menus.filter(menu => menu.category === category.value);
...
function renderReports() {
  ...
  const reportReservations = state.reservations.filter(item => isInReportRange(item, range));
...
function renderTypes() {
  ...
  root.innerHTML = `
    ...
    <section class="panel"><h2>Mevcut Organizasyon Türleri</h2><div class="category-grid">${state.organizationTypes.map(item => categoryCard(item, "organizationTypes")).join("") || empty("Henüz organizasyon türü eklenmemiş")}</div></section>
  `;
}
...
function renderMenuCategories() {
  ...
  root.innerHTML = `
    ...
    <section class="panel"><h2>Mevcut Menü Kategorileri</h2><div class="category-grid">${state.menuCategories.map(item => categoryCard(item, "menuCategories")).join("") || empty("Henüz menü kategorisi eklenmemiş")}</div></section>
  `;
}
...
function renderHalls() {
  ...
  root.innerHTML = `
    ...
    <section class="panel"><h2>Mevcut Salonlar</h2><div class="list">${state.halls.map(hallRow).join("") || empty("Yeni salon ekleyin")}</div></section>
  `;
}
...
function renderExtras() {
  ...
  root.innerHTML = `
    ...
    <section class="panel"><h2>Mevcut Özeller</h2><div class="list">${state.extras.map(extraRow).join("") || empty("Henüz özel seçim eklenmedi")}</div></section>
  `;
}
...
function quoteTotals(form) {
  ...
  const packageItem = state.packages.find(item => item.id === form.quotePackage?.value);
...
function bindForms() {
  ...
  document.querySelector("#detailedReservationForm")?.addEventListener("submit", event => {
    ...
    const totals = quoteTotals(form);
    const hall = state.halls.find(item => item.id === data.hallId);
    const packageItem = state.packages.find(item => item.id === data.packageId);
  ...
  document.querySelector("#reservationSearch")?.addEventListener("input", event => {
    ...
    const filtered = state.reservations.filter(item => reservationMatches(item, state.reservationQuery));
...
function addItem(collection, item) {
  state[collection] = [{ id: makeId(), ...item }, ...state[collection]];
  saveState();
  render();
}
...
document.addEventListener("click", event => {
  ...
  if (dayButton) {
    ...
    const existing = state.reservations.filter(item => item.date === date);
    state.editingReservationId = null;
